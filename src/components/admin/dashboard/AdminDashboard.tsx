...
import { AdminGusVerifier } from '../AdminGusVerifier';
import { AdminAntiSpam } from '../AdminAntiSpam';
import { AdminAuditLogger } from '../AdminAuditLogger';
import { AdminSecurityRadar } from '../AdminSecurityRadar';
import { AdminModerationQueue } from '../AdminModerationQueue';
import { AdminSystemTelemetry } from '../AdminSystemTelemetry';
  ...
  const loadRealAdminData = useCallback(async () => {
    ...
    try {
      const [uRes, cRes, aRes, pRes, catRes, rRes, repRes, lRes] = await Promise.allSettled([
        fetch('/api/mysql/users').then(r => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        }),
        fetch('/api/mysql/companies').then(r => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        }),
        fetch('/api/mysql/ads').then(r => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        }),
        fetch('/api/mysql/promotions').then(r => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        }),
        fetch('/api/mysql/categories').then(r => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        }),
