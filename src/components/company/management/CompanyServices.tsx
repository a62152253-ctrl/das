...
export function CompanyServices({ services, onAdd, onDelete, onUpdate }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim() || isNaN(parsedPrice) || parsedPrice <= 0) {
      addToast('Wprowadź poprawną nazwę oraz cenę większą od zera.', 'error');
      return;
    }
  ...
  return (
    <div className="font-sans">
      ...
      <div className={`grid grid-cols-1 ${hasServices ? 'lg:grid-cols-3' : 'lg:grid-cols-1'} gap-8`}>
        ...
        <form onSubmit={handleSubmit} className="bg-slate-50 border border-slate-200/50 p-6 rounded-2xl space-y-4 h-fit">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-2">Kategoria usługi</label>
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {services.length === 0 ? (
            <div className="text-center py-16 bg-white border-2 border-dashed border-slate-200/60 rounded-2xl">
              ...
              <h4 className="font-bold text-slate-900 text-sm mb-1">Brak usług w ofercie</h4>
              <p className="text-xs text-slate-500">Dodaj swoją pierwszą usługę za pomocą formularza po lewej stronie</p>
            </div>
                  ...
                  <div key={s.id} className={`p-4 transition-all hover:bg-slate-50/50 ${!s.isActive ? 'opacity-65' : ''}`}>
                    {isEditing ? (
                      <div className="space-y-4 bg-slate-50/50 p-3 rounded-lg border border-slate-200/60 animate-fadeIn">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs font-semibold text-slate-500 mb-2">Cena (zł)</label>
                            <input
                              type="number"
                              value={editPrice}
                          ...
                          <div>
                            <label className="block text-xs font-semibold text-slate-500 mb-2">Czas (min)</label>
                            <input
                              type="number"
                              value={editDuration}
